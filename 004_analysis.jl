...
# ╔═╡ 45f95bb5-195c-4b62-86eb-963cfc11c34d
...
# ╔═╡ 07979df7-5b8d-46d0-bb32-47f7b39dda2b
# ╠═╡ skip_as_script = true
#=╠═╡
TableOfContents(title="Analysis", depth=4)
  ╠═╡ =#

# ╔═╡ cacf753b-68a0-478e-bf51-23e82fe6b16d
# ╠═╡ skip_as_script = true
#=╠═╡
md"# Analysis"
  ╠═╡ =#
...
begin
	...
	i0_list = make_range(i0_r, 100, false)
	ρC_list = make_range(ρC_lr, 100, true)
	Z_list  = make_range(Z_lr, 100, true) .* MODEL.Zsun
	it_list = make_range(it_r, 100, false)
	
	# Reference value
	const ref_i0 = i0_list[50]
	const ref_ρC = ρC_list[90]
	const ref_Z  = Z_list[90]
	const ref_it = it_list[90]

	# ρ PDF parameters ranges
	const ALPHAS = range(1.0, 3.0, 20)
	const BS     = range(0.1, 1.0, 20)
	const MACHS  = range(1.0, 100.0, 20)

	pdf_par_a(a::Float64)::MODEL.PDF_params = MODEL.PDF_params(
		func=MODEL.PDF_PARAMS.func,
        α=a,
        b=MODEL.PDF_PARAMS.b,
        Ms=MODEL.PDF_PARAMS.Ms,
        deviation=(MODEL.PDF_PARAMS.deviation...,),
        divisions=MODEL.PDF_PARAMS.divisions,
    )

	pdf_par_b(b::Float64)::MODEL.PDF_params = MODEL.PDF_params(
		func=MODEL.PDF_PARAMS.func,
        α=MODEL.PDF_PARAMS.α,
        b=b,
        Ms=MODEL.PDF_PARAMS.Ms,
        deviation=(MODEL.PDF_PARAMS.deviation...,),
        divisions=MODEL.PDF_PARAMS.divisions,
	...
	pdf_par_ms(ms::Float64)::MODEL.PDF_params = MODEL.PDF_params(
		func=MODEL.PDF_PARAMS.func,
        α=MODEL.PDF_PARAMS.α,
        b=MODEL.PDF_PARAMS.b,
        Ms=ms,
...
# ╠═╡ skip_as_script = true
#=╠═╡
md"# Dependance with the ICs"
  ╠═╡ =#

# ╔═╡ 78e2ff6f-1409-468a-8416-59546da1d8d6
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Ionized fraction"
  ╠═╡ =#

# ╔═╡ 4f7f7e52-5c65-4c88-9151-9c393d26f2e5
# ╠═╡ skip_as_script = true
#=╠═╡
let
	
	integration_function = TESTING.integrate_with_julia(true)

	ics         = [[i0, 1.0 - i0, 0.0, 0.0] for i0 in i0_list]
	base_params = [ref_ρC, ref_Z]
	it          = ref_it

	sf = [integration_function(ic, base_params, it) for ic in ics]
	stellar_fraction = (sf .* exp10(7)) .- 358620
	...
	ax = Axis(
        ...
        ylabel=L"(\mathrm{stellar \,\, fraction} - 0.035862) \, \times \, 10^7",
		title=@sprintf(
    ...
    lines!(ax, i0_list, stellar_fraction, linewidth=3)
...
# ╔═╡ ea997fb7-83ba-4f86-91bb-33f86d8c47b8
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Density"
  ╠═╡ =#

# ╔═╡ 08d42be3-2320-42f9-a446-1b55589afb56
# ╠═╡ skip_as_script = true
#=╠═╡
let
	...
	ic          = [ref_i0, 1.0 - ref_i0, 0.0, 0.0]
	base_params = [[ρC, ref_Z] for ρC in ρC_list]
	it          = ref_it

	sf = [integration_function(ic, base_param, it) for base_param in base_params]
	...
	ax = Axis(
		...
		title=@sprintf(
			"i₀ = %.2g   Z = %.2f   it = %.2f Myr", ref_i0, ref_Z, ref_it,
		),
		titlesize=30,
		xlabelsize=32,
		ylabelsize=32,
...
# ╔═╡ 331646b1-2a60-4993-8f4a-64903a222bfc
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Metallicity"
  ╠═╡ =#

# ╔═╡ 67349df8-5c59-4fe3-889d-3571a8bbefa3
# ╠═╡ skip_as_script = true
#=╠═╡
let
	...
	ax = Axis(
        ...
        ylabel=L"\mathrm{stellar \,\, fraction}",
		title=@sprintf(
			"i₀ = %.2g   ρC = %.1f cm⁻³   it = %.2f Myr", ref_i0, ref_ρC, ref_it,
		),
		titlesize=30,
		xlabelsize=32,
		ylabelsize=32,
...
# ╔═╡ 596f0bc2-54fd-4eff-a23f-ca2ccd94c248
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Integration time"
  ╠═╡ =#

# ╔═╡ b823d38a-8667-4bc2-9e20-d5b3bf0246e2
# ╠═╡ skip_as_script = true
#=╠═╡
let
	...
	sf = [integration_function(ic, base_params, it) for it in its]
	...
	ax = Axis(
        ...
        ylabel=L"\mathrm{stellar \,\, fraction}",
		title=@sprintf(
			"i₀ = %.2g   ρC = %.1f cm⁻³   Z = %.2f", ref_i0, ref_ρC, ref_Z,
		),
		titlesize=30,
		xlabelsize=32,
		ylabelsize=32,
...
# ╔═╡ 6a27524a-f7f8-4753-b202-6e0463e0c386
# ╠═╡ skip_as_script = true
#=╠═╡
md"# Benchmarks"
  ╠═╡ =#

# ╔═╡ 3728a388-3672-44ca-81d1-85e2ef969f4f
# ╠═╡ skip_as_script = true
#=╠═╡
######################################################################################
...
# ╔═╡ 5412bc6d-fae0-48c7-b0cb-d53d9f6c68b8
# ╠═╡ skip_as_script = true
#=╠═╡
######################################################################################
...
function benchmark_csolve()::Nothing
	...
	integr_func = TESTING.integrate_with_c(true)
...
#=╠═╡
let
	integr_func = TESTING.integrate_with_c(false)

	densities = make_range((-6.0, 10.0), 100, true)
	sf = [
		integr_func(
			[ref_i0, 1.0 - ref_i0, 0.0, 0.0],
			[density, ref_Z],
    ...
    lines!(ax, densities, sf, linewidth=3)
...
begin
	...
	# Integration
    fractions = MODEL.integrate_model(ic, base_params, tspan; times)
end;

# ╔═╡ 7ad59526-d324-4ac4-a08b-ffa41336d6bf
function long_evolution(phase::String)::Figure
	...
	ax = Axis(
		...
		title=@sprintf(
			"i₀ = %.2g   ρC = %.1f cm⁻³   Z = %.2f   it = %.2f Myr", 
			ref_i0, ref_ρC, ref_Z, ref_it,
		),
		titlesize=28,
		xlabelsize=32,
		ylabelsize=32,
...
# ╔═╡ 99f3938a-bbc5-4436-9af8-e12d9397bd72
# ╠═╡ skip_as_script = true
#=╠═╡
long_evolution("ionized")
  ╠═╡ =#

# ╔═╡ 6b2fe861-6da9-4b5c-972a-90a04ab0930f
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Atomic fraction"
  ╠═╡ =#

# ╔═╡ d4498dc6-cc07-4094-b2b1-d4fe02c5571e
# ╠═╡ skip_as_script = true
#=╠═╡
long_evolution("atomic")
  ╠═╡ =#

# ╔═╡ 4681a84c-77ad-4503-8339-d324a12f20e4
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Molecular fraction"
  ╠═╡ =#

# ╔═╡ 3581011d-6d14-4487-a8e1-c4219c2f3e0d
# ╠═╡ skip_as_script = true
#=╠═╡
long_evolution("molecular")
  ╠═╡ =#

# ╔═╡ 3d8c942c-0c88-403b-ac22-0ffeda40d94f
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Stellar fraction"
  ╠═╡ =#

# ╔═╡ 1e04f9a4-5523-4c65-8ff0-3138e0a9b131
# ╠═╡ skip_as_script = true
#=╠═╡
long_evolution("stellar")
  ╠═╡ =#

# ╔═╡ 73622913-6c2b-4236-9104-c67c13302e73
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Star formation time"
  ╠═╡ =#

# ╔═╡ b3130782-4e64-46f9-b367-47f1745f6000
# ╠═╡ skip_as_script = true
#=╠═╡
let
    τS = MODEL.τS.(
...
# ╔═╡ c0dcd9b9-41dd-455e-ba33-57231259e1c1
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Recombination time"
  ╠═╡ =#

# ╔═╡ 709923b8-7328-4b74-969e-335b6b0b4899
# ╠═╡ skip_as_script = true
#=╠═╡
let
    τR = MODEL.τR.(
...
# ╔═╡ 9e768138-f55a-492c-bdbc-17e7e6da76b9
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Condensation time"
  ╠═╡ =#

# ╔═╡ d2e6361a-f9ae-4982-9cf0-480b309bda8e
# ╠═╡ skip_as_script = true
#=╠═╡
let
    τC = MODEL.τC.(
...
# ╔═╡ 67bb7505-d0ff-46f0-9baa-879bf608dd10
# ╠═╡ skip_as_script = true
#=╠═╡
md"# Dependance with the $\rho$ PDF parameters"
  ╠═╡ =#

# ╔═╡ 38576d0d-7703-4224-b1ec-806c6c5a5681
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Power law slope (α)"
  ╠═╡ =#

# ╔═╡ 253dc9e7-725a-447c-b84d-ef03eb5139ed
# ╠═╡ skip_as_script = true
#=╠═╡
let
	functions = [
		TESTING.integrate_with_julia(true; pdf_params=pdf_par_a(alpha)) for 
		alpha in ALPHAS
	]
	...
	ax = Axis(
        ...
        ylabel=L"\mathrm{stellar \,\, fraction}",
		title=@sprintf(
			"b = %.1f   Ms = %.1f  it = %.2f Myr", 
			MODEL.PDF_PARAMS.b, MODEL.PDF_PARAMS.Ms, ref_it,
		),
		titlesize=28,
		xlabelsize=32,
		ylabelsize=32,
	...
	lines!(ax, ALPHAS, sf, linewidth=3)

    f
end
  ╠═╡ =#

# ╔═╡ 04bc4c64-1b05-4474-abc1-fae9a8b32548
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Turbulent forcing parameter (b)"
  ╠═╡ =#

# ╔═╡ 9ed8de5b-fa7c-4750-bd44-3d8cd2ed0826
# ╠═╡ skip_as_script = true
#=╠═╡
let
	functions = [
		TESTING.integrate_with_julia(true; pdf_params=pdf_par_b(b)) for 
		b in BS
	]
	...
	ax = Axis(
        ...
        ylabel=L"\mathrm{stellar \,\, fraction}",
		title=@sprintf(
			"α = %.1f   Ms = %.1f  it = %.2f Myr", 
			MODEL.PDF_PARAMS.α, MODEL.PDF_PARAMS.Ms, ref_it,
		),
		titlesize=28,
		xlabelsize=32,
		ylabelsize=32,
	...
	lines!(ax, BS, sf, linewidth=3)

    f
end
  ╠═╡ =#

# ╔═╡ cdd969b7-532a-4863-891c-244c1c1fe892
# ╠═╡ skip_as_script = true
#=╠═╡
md"## Mach number (Ms)"
  ╠═╡ =#

# ╔═╡ 177301af-29d0-4f06-9349-5e2f1c545938
# ╠═╡ skip_as_script = true
#=╠═╡
let
	functions = [
		TESTING.integrate_with_julia(true; pdf_params=pdf_par_ms(ms)) for 
		ms in MACHS
	]
	...
	ax = Axis(
		...
		title=@sprintf(
			"α = %.1f   b = %.1f  it = %.2f Myr", 
			MODEL.PDF_PARAMS.α, MODEL.PDF_PARAMS.b, ref_it,
		),
		titlesize=28,
		xlabelsize=32,
		ylabelsize=32,
...
#         ylabel=L"\mathrm{M_\star / M_{celda}}",
# 		title=@sprintf(
# 			"i₀ = %.2g  a₀ = %.2g  ρ = %.1f cm⁻³  it = %.1f Myr", 0.5, 0.5, 4000, 10,
# 		),
# 		titlesize=40,
# 		xlabelsize=42,
# 		ylabelsize=42,
...
#     lines!(ax, Z_list, s_f, linewidth=6)

#     CairoMakie.save("C:/Users/Ezequiel/Desktop/model_plots/star_fraction_vs_metalicity.png", f)
...
#     lines!(ax, Z_list, m_f, linewidth=6)

#     CairoMakie.save("C:/Users/Ezequiel/Desktop/model_plots/molecular_fraction_vs_metalicity.png", f)
...
#     lines!(ax, Z_list, a_f, linewidth=6)

#     CairoMakie.save("C:/Users/Ezequiel/Desktop/model_plots/atomic_fraction_vs_metalicity.png", f)
...
#     lines!(ax, Z_list, i_f, linewidth=6)

#     CairoMakie.save("C:/Users/Ezequiel/Desktop/model_plots/ionized_fraction_vs_metalicity.png", f)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "9018f714182b3a05158dae0fc406ad6b487b2bf0"
