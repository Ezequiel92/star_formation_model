...
begin
	...
	const cs  = ustrip(t_u * l_u^-(3/2), CS / sqrt(u"mp"))
end
...
let
	...
	axislegend(; position=:rt, labelsize=22)
...
begin
	...
	const CR = u"mp" / αB
	const cr = ustrip(t_u * l_u^-3, CR / u"mp")
end
...
# ╠═╡ skip_as_script = true
#=╠═╡
md"### Density PDF by Krumholz (2005)"
  ╠═╡ =#

# ╔═╡ aad5e227-67a5-49a0-a79d-24160d3ebe06
######################################################################################
# Density PDF acording to Krumholz et al. (2018)
# https://doi.org/10.1086/431734
######################################################################################

function pKrumholz2005(s::Float64, params::PDF_params)::Float64
...
begin
    const PDF_PARAMS = PDF_params(
		func = pKrumholz2005,      # Density PDF function 
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "a4cf857425ebeaa15c2de508385d70c9a4a1574d"
