...
let
	with_theme(merge(theme_latexfonts(), THEME)) do
		...
		Makie.save(
			"../plots/parameters/R-vs-metalicity.pdf",
			f,
			pt_per_unit=0.28346,
			px_per_unit=1.0,
		)
...
let
	with_theme(merge(theme_latexfonts(), THEME)) do
		...
		Makie.save(
			"../plots/parameters/Zsn-vs-metalicity.pdf",
			f,
			pt_per_unit=0.28346,
			px_per_unit=1.0,
		)
...
manifest_format = "2.0"
project_hash = "f4ee25ff7fbffc0dd77704e614183107e251931d"
