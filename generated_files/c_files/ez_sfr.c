...
static int jacobian(double t, const double y[], double *dfdy, double dfdt[], void *parameters)
{
	...
	*/
	...
	gsl_matrix_set(m, 0, 1, (0.019428762831580126 * y[2] * rho_C * (eta_i + R)) / (2 * aux_var));
	gsl_matrix_set(m, 0, 2, (0.019428762831580126 * y[2] * rho_C * (eta_i + R)) / (2 * aux_var) + 0.019428762831580126 * (eta_i + R) * aux_var);
	gsl_matrix_set(m, 0, 3, 0);

	gsl_matrix_set(m, 1, 0, 16.409952 * y[0] * rho_C);
	gsl_matrix_set(m, 1, 1, (0.019428762831580126 * y[2] * rho_C * (eta_d - eta_i)) / (2 * aux_var) - 17.393952755905513 * y[1] * rho_C * (1.27e-5 + Z) - 17.393952755905513 * (y[1] + y[2]) * rho_C * (1.27e-5 + Z));
	gsl_matrix_set(m, 1, 2, (0.019428762831580126 * y[2] * rho_C * (eta_d - eta_i)) / (2 * aux_var) + 0.019428762831580126 * (eta_d - eta_i) * aux_var - 17.393952755905513 * y[1] * rho_C * (1.27e-5 + Z));
	gsl_matrix_set(m, 1, 3, 0);

	gsl_matrix_set(m, 2, 0, 0);
	gsl_matrix_set(m, 2, 1, (-0.019428762831580126 * y[2] * rho_C * (1 + eta_d)) / (2 * aux_var) + 17.393952755905513 * y[1] * rho_C * (1.27e-5 + Z) + 17.393952755905513 * (y[1] + y[2]) * rho_C * (1.27e-5 + Z));
	gsl_matrix_set(m, 2, 2, (-0.019428762831580126 * y[2] * rho_C * (1 + eta_d)) / (2 * aux_var) - 0.019428762831580126 * (1 + eta_d) * aux_var + 17.393952755905513 * y[1] * rho_C * (1.27e-5 + Z));
	gsl_matrix_set(m, 2, 3, 0);

	gsl_matrix_set(m, 3, 0, 0);
	gsl_matrix_set(m, 3, 1, (0.019428762831580126 * y[2] * rho_C * (1 - 1 * R)) / (2 * aux_var));
	gsl_matrix_set(m, 3, 2, (0.019428762831580126 * y[2] * rho_C * (1 - 1 * R)) / (2 * aux_var) + 0.019428762831580126 * (1 - 1 * R) * aux_var);
