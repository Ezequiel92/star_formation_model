static int jacobian(double t, const double y[], double *dfdy, double dfdt[], void *parameters)
{
	...
	*/
	...
	gsl_matrix_set(m, 1, 1, 0.2826053731343284 * (1.34e-5 + Z) * (-1 + y[3]) * rho_C);
	gsl_matrix_set(m, 1, 2, 0.0003885752566316025 * (-1 * eta_i + eta_d) * aux_var);
	gsl_matrix_set(m, 1, 3, (-0.0003885752566316025 * (-1 * eta_i + eta_d) * y[2] * rho_C) / (2 * aux_var) + 0.2826053731343284 * (1.34e-5 + Z) * y[1] * rho_C);

	gsl_matrix_set(m, 2, 0, 0);
	gsl_matrix_set(m, 2, 1, 0.2826053731343284 * (1 - y[3]) * (1.34e-5 + Z) * rho_C);
	gsl_matrix_set(m, 2, 2, 0.0003885752566316025 * (-1 - eta_d) * aux_var);
	gsl_matrix_set(m, 2, 3, (-0.0003885752566316025 * (-1 - eta_d) * y[2] * rho_C) / (2 * aux_var) - 0.2826053731343284 * (1.34e-5 + Z) * y[1] * rho_C);
