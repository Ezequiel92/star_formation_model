static int jacobian(double t, const double y[], double *dfdy, double dfdt[], void *parameters)
{
	...
	*/
	...
	gsl_matrix_set(m, 1, 1, 0.29818204724409453 * (-1 + s_f) * (1.27e-5 + Z) * rho_C);
	gsl_matrix_set(m, 1, 2, 0.0003885752566316025 * (-1 * eta_i + eta_d) * aux_var);
	gsl_matrix_set(m, 1, 3, (-0.0003885752566316025 * (-1 * eta_i + eta_d) * m_f * rho_C) / (2 * aux_var) + 0.29818204724409453 * (1.27e-5 + Z) * a_f * rho_C);

	gsl_matrix_set(m, 2, 0, 0);
	gsl_matrix_set(m, 2, 1, 0.29818204724409453 * (1 - s_f) * (1.27e-5 + Z) * rho_C);
	gsl_matrix_set(m, 2, 2, 0.0003885752566316025 * (-1 - eta_d) * aux_var);
	gsl_matrix_set(m, 2, 3, (-0.0003885752566316025 * (-1 - eta_d) * m_f * rho_C) / (2 * aux_var) - 0.29818204724409453 * (1.27e-5 + Z) * a_f * rho_C);
